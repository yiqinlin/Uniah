...
public class UniStatusBarHelper {
    ...
    private static boolean supportTranslucent() {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT
                ...
                && !(UniDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT < 26);
    }
    ...
    public static void translucent(Window window, @ColorInt int colorOn5x) {
        ...
        if (UniNotchHelper.isNotchOfficialSupport()) {
            handleDisplayCutoutMode(window);
        }

        // 小米和魅族4.4 以上版本支持沉浸式
        // 小米 Android 6.0 ，开发版 7.7.13 及以后版本设置黑色字体又需要 clear FLAG_TRANSLUCENT_STATUS, 因此还原为官方模式
        if (UniDeviceHelper.isMeizu() || (UniDeviceHelper.isMIUI() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M)) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && supportTransclentStatusBar6()) {
                ...
                Log.e("mActivity", "isB:" + window.getStatusBarColor());
            } else {
...
//                window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
                ...
                Log.e("mActivity", "isA:" + window.getStatusBarColor());
            }
...
//            }
    ...
    public static boolean isTranslucent(Window window) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            Log.e("mActivity", "is:" + window.getStatusBarColor());
            return (window.getAttributes().flags & WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) == WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS && window.getStatusBarColor() != -1;
        }
        return false;
    }

    public static int getTranslucentHeight(Activity activity) {
        int statusBarHeight = 0;
        if (activity != null && UniStatusBarHelper.isTranslucent(activity)) {
            statusBarHeight = UniNotchHelper.getSafeInsetTop(activity);
            if (statusBarHeight == 0) {
                statusBarHeight = UniStatusBarHelper.getStatusbarHeight(activity);
            }
        }
    ...
    private static boolean isMIUICustomStatusBarLightModeImpl() {
        if (UniDeviceHelper.isMIUIV9() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            return true;
        }
        return UniDeviceHelper.isMIUIV5() || UniDeviceHelper.isMIUIV6() ||
                UniDeviceHelper.isMIUIV7() || UniDeviceHelper.isMIUIV8();
    }
    ...
    public static boolean supportTransclentStatusBar6() {
        return !(UniDeviceHelper.isZUKZ1() || UniDeviceHelper.isZTKC2016());
    }
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (UniDeviceHelper.isTablet(context)
                && sStatusbarHeight > UniDisplayHelper.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP)) {
