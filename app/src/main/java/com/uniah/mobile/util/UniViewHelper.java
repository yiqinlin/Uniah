...
public class UniViewHelper {
    ...
    public static void playViewBackgroundAnimation(final View v, @ColorInt int startColor, @ColorInt int endColor, long duration, int repeatCount, int setAnimTagId, final Runnable endAction) {
        ...
        UniViewHelper.setBackgroundColorKeepPadding(v, startColor);
        ...
        anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                UniViewHelper.setBackgroundColorKeepPadding(v, (Integer) animation.getAnimatedValue());
            }
        });
        if (setAnimTagId != 0) {
     ...
     * <p>相关方法 {@link #slideOut(View, int, Animation.AnimationListener, boolean, UniDirection)}</p>
     *
     * @param view            做动画的 View
     * @param duration        动画时长(毫秒)
     * @param listener        动画回调
    ...
    public static
    @Nullable
    TranslateAnimation slideIn(final View view, int duration, final Animation.AnimationListener listener, boolean isNeedAnimation, UniDirection direction) {
        if (view == null) {
            return null;
        }
        if (isNeedAnimation) {
     ...
     * <p>相关方法 {@link #slideIn(View, int, Animation.AnimationListener, boolean, UniDirection)}</p>
     *
     * @param view            做动画的 View
     * @param duration        动画时长(毫秒)
     * @param listener        动画回调
    ...
    public static
    @Nullable
    TranslateAnimation slideOut(final View view, int duration, final Animation.AnimationListener listener, boolean isNeedAnimation, UniDirection direction) {
        if (view == null) {
            return null;
        }
        if (isNeedAnimation) {
     ...
     * if (UniViewHelper.getISLastLineSpacingExtraError()) {
    ...
    /**
     * requestDisallowInterceptTouchEvent 的安全方法。存在它的原因是 UniPullRefreshLayout 会拦截这个事件
     *
     * @param view
     * @param value
     */
    public static void safeRequestDisallowInterceptTouchEvent(@NonNull View view, boolean value) {
        ...
        if (viewParent != null) {
            ...
            while (layout != null) {
                if (layout instanceof UniPullRefreshLayout) {
                    ((UniPullRefreshLayout) layout).openSafeDisallowInterceptTouchEvent();
                }
